...
- **Increased contrast is covered by construction, not by that test.** `NSAppearance(named: .accessibilityHighContrastAqua)` resolves to the ordinary palette unless the system-wide Increase Contrast setting is on, which a test process cannot set — so a test naming those appearances would re-measure the normal palette and report coverage it does not have. It matters less than it sounds: in increased contrast `NotificationRow` does not tint the row at all, it strokes a 2 pt border in the token color, so there is no text-over-tint stack there to measure. The border's own contrast against the row surface still wants checking by hand with Increase Contrast enabled.
