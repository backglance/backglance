...
- **Highlight colors from Rules are the risk point.** Rule highlight tokens (`amber`, etc.) resolve to system colors, which already carry light, dark and increased-contrast variants (see [RULES.md](../features/RULES.md#rule-kinds) for why there is no asset catalog). They are used as *row background tints* behind primary label color — never as text color. Each token's tint is capped at `HighlightColor.tintOpacity`, the opacity where `Color.primary` on top of it stays ≥ 4.5:1; `HighlightContrastTests` resolves every token against every row surface in light and dark and asserts exactly that, reading the shipped constant rather than a copy of it, so raising the tint fails the test.
- **Increased contrast needs the system setting, not an appearance name.** `NSAppearance(named: .accessibilityHighContrastAqua)` returns an object whose `name` is `NSAppearanceNameAqua`: the high-contrast names collapse to the plain ones unless Increase Contrast is on system-wide, and the palette follows `NSWorkspace.accessibilityDisplayShouldIncreaseContrast` instead. A test naming those appearances would re-measure the ordinary palette under a misleading label.
- **The highlight border is measured, but only when that setting is on.** In increased contrast `NotificationRow` does not tint the row at all — it strokes a 2 pt border in the token color, which makes that border the whole highlight signal and puts it under WCAG 1.4.11's 3:1 bound for non-text elements. `HighlightContrastTests.testHighlightBorderClearsNonTextContrast` asserts exactly that and skips, with instructions, when Increase Contrast is off. It has been run with the setting on: every token clears the bar with margin — **light 4.54–5.21, dark 5.55–8.86**. The check is worth keeping precisely because the ordinary palette does *not* clear it (amber 2.31, green 2.22); what saves it is that Apple's accessible variants are markedly darker, and those are what paint in this mode.
