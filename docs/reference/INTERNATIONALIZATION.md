...
For counts in `String(localized:)` contexts, use the same catalog plural support rather than `count == 1 ? ... : ...` — the ternary hardcodes English grammar. Every count in the project now does (the last three hand-branched sites — `ExportSheet`, `UndoToastView`, `StatusItemAccessibility` — converted once BACKGLANCE-250 gave each a rendered-output test).

**Where plurals are verified — and where they cannot be.** No unit test bundle in this project can see a catalog plural render: none has a `TEST_HOST`, so `Bundle.main` inside them is the xctest runner, every lookup falls back to the key, and an assertion there proves the key, not the grammar — a conversion once shipped the singular to VoiceOver with eight green unit tests (BACKGLANCE-238). That is a deliberate trade, not an accident to fix: hosting the unit bundles inside `Backglance.app` would boot the whole agent app (archive, capture engine, status item) under every unit run, and moving the catalog into a package would break the one-catalog design above. Rendered strings are asserted in the one bundle that launches the real app instead — `BackglanceAppUITests`, where `PluralRenderingTests` reads all three converted sites off the running app and `OnboardingFDATests` covers the import line. Any future plural conversion must arrive with a test there; unit tests may assert only what survives the fallback (that the count is interpolated at all).
