...
```swift
// Packages/BackglanceCapture/Sources/BackglanceCapture/Adapters/StoreFingerprints.swift
import Foundation
...
public enum StoreFingerprints {
    struct File: Decodable, Sendable {
        var version: Int
        var adapters: [String: [String]]   // keyed by adapter id, so a new adapter needs no code here
    }

    /// Hashes verified for the adapter with this id, or an empty set if it has none yet.
    public static func hashes(forAdapter id: String) -> Set<String> { byAdapter[id] ?? [] }

    public static var v14: Set<String> { hashes(forAdapter: StoreAdapterV14.id) }
    public static var v15: Set<String> { hashes(forAdapter: StoreAdapterV15.id) }
    public static var v26: Set<String> { hashes(forAdapter: StoreAdapterV26.id) }

    private static let byAdapter: [String: Set<String>] = {
        guard let url = resourceURL else {
            // A missing resource is a packaging bug: assert in DEBUG, and in RELEASE fall
            // through to no known hashes — every store then takes the OS-major fallback
            // and its probe, which is safe, rather than crashing on launch.
        ...
        do { return try decode(Data(contentsOf: url)) } catch {
            assertionFailure("KnownFingerprints.json could not be decoded")
            return [:]
        }
    }()
}
